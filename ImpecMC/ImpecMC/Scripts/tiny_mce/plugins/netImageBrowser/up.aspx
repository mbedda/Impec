...
<script runat="server">
    ...
    private string path = HttpContext.Current.Server.MapPath("../../../../Content/upimages");
    private string imgPath = "../../../../Content/upimages";
            ...
            $('a[rel*=jmenu]').contextMenu({
                menu: 'myMenu'
					...
					function (action, el, pos) {
					    ...
					    if (action == "paste") {
					        var appPath = '<%= string.Format("{0}://{1}{2}{3}", Request.Url.Scheme, Request.Url.Host, Request.Url.Port == 80 ? string.Empty : ":" + Request.Url.Port, Request.ApplicationPath) %>';
					        if (appPath.substr(-1) != "/")
					            appPath += "/";
                            var path = appPath + "Content/upimages/";
					        var queryString = $.jqURL.get("fl");
